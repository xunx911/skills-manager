...
5. Run matrix 已经提供 read-only 多 run x case 浏览、保存筛选视图、对照/候选 impact、impact 过滤和分组、`Impact` 列隐藏以及当前视图 CSV 导出；但还没有任意列配置、自定义指标列或保存对照/候选 run 指针。
...
5. 做 run matrix 多维表格后续：任意列配置、自定义指标列，并考虑是否保存对照/候选 run 指针。

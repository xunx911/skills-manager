...
9. 在 `测评` 页可以直接用快速添加面板录入单条 case，或切到 `批量` 后粘贴多行 `title | input | expected output | notes`。如果某行缺字段，页面会阻止提交、聚焦错误摘要，并在批量文本框旁提示第几行需要修正。
